...
>
> ⭐ **The rule is satisfied across this section as of `0.8.2.32`**, which is a later fold than the one that landed it: `0.8.2.31` stated the rule and applied it to the two rows it was investigating, leaving the other twenty-five it governs untouched. **A sweep corrects the instances it was looking at** — the identical shape `0.8.2.31` itself diagnosed one revision earlier, and reported by an outside seat both times.
>
> ⚠ **A row that CITES a section is not thereby a restatement, and two rows here are not.** The `0.8.2.13` withdrawal note cites §6.2 and §6.13 for where install authorization now lives, but **the withdrawal itself is recorded nowhere else** — this list is its home, and pointing a reader elsewhere for it would be the defect. Deciding which a row is **requires opening the cited section**, which is why no gate rules on this: `spec pointers --floor` reports candidates and never a verdict.
...
- **The subject a handler acts on is drawn from `effective_targets` (§5.2), never `resource.targets[0]`** — `subject ⊆ effective_targets`, and for a resource-requiring operation `effective[0]` with §3.3's cardinality rule selecting `path_required` / `ambiguous_resource` / `malformed_resource`. Implementing the cardinality check alone leaves the bypass open (0.8.2.20) **§5.2 is the normative home for the derivation and §3.3's 400 row for the cardinality selection; this row is a restatement of both (0.8.2.32).**
- **`canonicalize` is total and every consumer of `NEVER_MATCH` is ruled (§5.4)** — `matches_pattern` returns false on either operand; `validate_absolute_path` errors; such a path is never stored or resolved. **Every `validate_absolute_path` call site consumes its verdict and fails closed** (0.8.2.20) **§5.4 is the normative home and this row is a restatement (0.8.2.32).**
...
- Dispatch routing (§1.4) — reject an inbound EXECUTE targeting a non-self peer_id with **400 `invalid_request`** (0.8.2.2 names the code; the status was already pinned). The refusal runs at canonicalization, **before** handler resolution and before `check_permission` (§6.5 step 3), so it is pre-authorization: it MUST NOT be reported as `404 handler_not_found` (§6.2) or `403 capability_denied`, and MUST NOT be reached by resolving a local handler for the foreign path and letting §5.2 decide. Enumerated in the §5.2a pre-dispatch row; driven by `dispatch_inbound_foreign_namespace_refused` **§1.4's `Inbound dispatch (wire listener)` is the normative home and this row is a restatement (0.8.2.32).**
- **Unimplemented operation on a registered handler** (§3.3 501 row, §6.2) — dispatching an operation absent from a registered handler's manifest emits **501 `unsupported_operation`**. The code slot at 501 carries no synonym **of this row**: `unknown_operation`, `not_implemented`, `not_supported` and `not_available` are all non-conformant spellings of it (0.8.2.7). Distinct from 404, where no handler is registered, and from 403, where the caller's authority is the failing input — **and where both are true, 403 wins, because `check_permission` runs first (§6.5, §6.7; 0.8.2.30). A check set MUST drive this row with a grant that COVERS the probed operation**, or it measures the ordering instead of the row and scores a conformant peer against a requirement it does not name. **A domain code defined for a different failure that also answers 501 is not a synonym and is not blacklisted (0.8.2.8)** — `unsupported_mode` (`EXTENSION-REGISTRY` §6a.9.2, live registration under a stored but unenforceable `domain-control` policy) is the worked case: the handler is registered and the operation *is* implemented, so it is not this row at all. The test is the failure named, never the status shared **§3.3's 501 row is the normative home and this row is a restatement; §6.2's statement is itself a restatement of §3.3 and says so (0.8.2.32).**
- **No handler at the dispatch path** (§3.3 404 row, §6.2) — dispatching to a path with no registered handler, **on a path targeting the local peer**, emits **404 `handler_not_found`** (0.8.2.7). A 404 raised inside a registered handler for an absent entity, binding or hash is a domain outcome and is not this row; a path targeting a foreign namespace is refused earlier with **400 `invalid_request`** (§5.2a) **§6.2 is the normative home and this row is a restatement; §3.3's 404 row is itself a restatement of §6.2 and says so — the two codes point OPPOSITE ways between these sections (0.8.2.32).**
- Connection handler ordering enforcement (§4.2) — hello before authenticate; an `authenticate` before a hello nonce was issued emits **401 `invalid_nonce`** (§4.6 step 1, §4.7 row 6), never `connection_sequence_error`. **§4.6 step 1 is the normative home for the rule and §4.7 row 6 for the code; this row is a restatement of both (0.8.2.32).**
- Protocol negotiation (§4.5) — a hello whose non-empty `protocols` set does not intersect the responder's emits **400 `incompatible_protocol`**; a hello with `protocols` **absent or empty** emits **400 `invalid_request`**. The identifiers intersected are §8.4's. **§4.5 is the normative home for the negotiation and §4.7 for the codes; this row is a restatement of both (0.8.2.32).**
- Connect operation validity (§4.7) — an operation the responder implements, arriving in a forbidden state, emits **409 `connection_sequence_error`**; an operation it does not implement emits **400 `invalid_request`**. **§4.7 is the normative home and this row is a restatement (0.8.2.32).**
- Proof-of-possession step order (§4.6) — for an input failing more than one numbered step, the emitted code is the **lowest-numbered** failing step's. **§4.6 is the normative home and this row is a restatement (0.8.2.32).**
- Connection uniqueness (§4.2) — reject subsequent connection requests with status 409. **§4.2 is the normative home and this row is a restatement; the code is §4.7's `connection_already_established` (0.8.2.32).**
- Connect-time proof-of-possession (§4.6) — nonce-echo (401 `invalid_nonce`), signature verification against `authenticate.public_key` (401 `authentication_failed`), and peer_id↔public_key identity binding (401 `identity_mismatch`); `hello`/`authenticate` peer_id equality on the same connection. **§4.6 is the normative home and this row is a restatement (0.8.2.32).**
- Auth boundary status (§4.6, §5.2, §8.3) — connect-auth failures emit coded 401 *before* close; request-time `verify_request` DENY maps to 403 (except `unresolvable_grantee` → 401). **§8.3's authentication-boundary paragraph is the normative home and this row is a restatement (0.8.2.32).**
- Connection error-code contract (§4.7) — emit the pinned `result.data.code`/status for each connect failure (clients key off the code). **§4.7 is the normative home and this row is a restatement (0.8.2.32).**
