...
system/bounds := {
  fields: {
    ...
    chain_id:       {type_ref: "primitive/string", optional: true}
                     ...
                     ; (v7.76 — pins the format; a generated UUID is the default, not
    ...
    chain_depth:    {type_ref: "primitive/uint", optional: true}
                     ; Causal continuation-advancement depth (v7.76), parallel to
...
- EXECUTE targeting any other path without valid authentication MUST be rejected per §5.2a: a missing/unverifiable `author` or signature is auth-class **401**; an authenticated request lacking a covering capability is authz-class **403** (v7.76, F32 — this rule previously said a blanket 403, contradicting §4.4/§5.2a).
...
0. **Key-type support (v7.76, UN-b/F48).** The responder MUST validate that `authenticate.key_type` is a supported, allocated key_type (§1.5) **before** identity binding. An unsupported key_type MUST be rejected with **400 `unsupported_key_type`**, not `401 identity_mismatch` — the algorithm is unsupported; the identity was never mismatched. Producing this status for an experimental allocation (`0xF0–0xFD`, §1.5, which carries no per-code string) requires decoding the varint key_type. (Hoisted from the Hardening block below, where the MUST sat un-ordered; a peer running the numbered steps in order returned `401 identity_mismatch` for an unknown key_type — Unison `A-UN-016`.)
...
Every authenticated EXECUTE MUST include `author` and `capability` in data, plus a valid signature in the envelope. A request **missing or carrying an unverifiable `author` / signature** is an authentication failure → **401** (auth-class); a request that authenticates but **lacks a covering `capability`** is an authorization failure → **403 `capability_denied`**. The §5.2a enumeration is authoritative for the (status, code) tuple (v7.76, F32 — this clause previously said a blanket `403`, contradicting the §5.2a discriminator).
...
**Namespace disambiguation (v7.76, UN-a/F47).** This policy-path `{peer_pattern}` — a content-hash **hex** segment closed to exactly the two forms above (invariant-pointer hex or the literal `default`) — is a **distinct namespace** from the capability **`peers:` scope** IdScope patterns (`peers: include = ["/{peer_id}/…"]`, §5.2), which are peer-id **Base58** and *do* accept the transitional pending-canonicalization form (§1.4 rule-3). They collide only in the informal name "peer pattern." The §3018 hex-only closure (no partial-prefix matchers) MUST NOT be applied to the `peers:` scope patterns, and the Base58 scope form MUST NOT be accepted at this policy path. (Keystone's `PEER-PATTERN-2` proposed adding the Base58 form *here* — **rejected**: it would reopen the §3018 typo-attack closure. If that oracle tests a Base58 form against the policy path it over-demands; the fix is an oracle correction routed back to keystone, not a spec change. Both namespaces are already individually correct.)
