...
| **Non-canonical ECF — bytes that DO decode as CBOR and are not canonical.** Members include a CBOR major-type-6 item in a data-field position, an indefinite length, a non-minimal integer, and map keys out of §1.3 order | **400** `non_canonical_ecf` | §4.7; §1.11; `ENTITY-CBOR-ENCODING.md` §9.2, §6.3 |
