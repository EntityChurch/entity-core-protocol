...
0. **Key-type support (0.8.1, UN-b/F48).** The responder MUST validate that `authenticate.key_type` is a supported, allocated key_type (§1.5) **before** identity binding. An unsupported key_type MUST be rejected with **400 `unsupported_key_type`**, not `401 identity_mismatch` — the algorithm is unsupported; the identity was never mismatched. Producing this status for an experimental allocation (`0xF0–0xFD`, §1.5, which carries no per-code string) requires decoding the varint key_type. (Hoisted from the Hardening block below, where the MUST sat un-ordered; a peer running the numbered steps in order returned `401 identity_mismatch` for an unknown key_type — Unison `A-UN-016`.)
...
`ttl` and the continuation `chain_depth` ceiling (recommended **64**, §3.11 / EXTENSION-CONTINUATION §3.9) MUST be **distinct** magnitudes with `chain_depth` ceiling ≤ `ttl` seed; the pinned **ratio is 8×** (a cohort measurement MAY retune it — the *ratio*, not the bare seed, is the conformance-relevant quantity). Equal magnitudes (both were 64) let TTL mask the deterministic depth brake — the "9-vs-64" cross-peer bound divergence this pins out (0.8.1).
